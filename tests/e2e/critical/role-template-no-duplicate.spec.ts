...
import { apiHealthOk, getSettings, putSettingsBulk } from '../utils/api-client';
...
test.describe('@critical Role template anti-duplicate', () => {
  ...
  test('bulk PUT with duplicate role rows collapses to one in GET', async ({ ownerToken }) => {
    const before = (await getSettings(ownerToken)) as { roleTemplates?: unknown[] };
    ...
    await putSettingsBulk(ownerToken, { ...before, roleTemplates: dupes });
