...
create table if not exists public.membership_plan_catalog (
  ...
  constraint membership_plan_catalog_gym_plan_uidx unique (gym_id, plan_name),
...
create index if not exists membership_plan_catalog_gym_sort_idx
  on public.membership_plan_catalog (gym_id, sort_order, plan_name);

comment on table public.membership_plan_catalog is
  'Staff-facing membership plan showcase metadata. Does not drive member amounts or Finance.';
...
create policy membership_plan_catalog_select_gym
  on public.membership_plan_catalog
  for select
  to authenticated
...
drop policy if exists membership_plan_catalog_write_owner
  on public.membership_plan_catalog;
create policy membership_plan_catalog_write_owner
  on public.membership_plan_catalog
  for all
  to authenticated
  using (
