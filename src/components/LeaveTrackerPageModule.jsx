...
export default function LeaveTrackerPageModule({ users, settings, updateSetting, currentUser, focusLeaveRequestId = '', focusLeaveUserId = '', onClearFocus }) {
  ...
  return (
    <div className="p-3 md:p-8 space-y-4">
      <div>
        <h1 className="text-xl md:text-2xl font-semibold text-slate-900">Leave Tracker</h1>
        <p className="text-sm text-slate-600">Leave requests, approvals, and yearly balance tracking.</p>
      </div>
      <div className="rounded-3xl border border-slate-200 bg-white p-4 space-y-3">
        <h3 className="text-base font-semibold">Create Leave Request</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          <div><label className="text-xs text-slate-500">Staff</label><select value={form.userId} onChange={(e) => setForm((v) => ({ ...v, userId: e.target.value }))} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm">{staff.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
          <div><label className="text-xs text-slate-500">Type</label><select value={form.type} onChange={(e) => setForm((v) => ({ ...v, type: e.target.value }))} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"><option>Casual</option><option>Sick</option><option>Emergency</option><option>Unpaid</option></select></div>
          <div><label className="text-xs text-slate-500">Start</label><input type="date" value={form.startDate} onChange={(e) => setForm((v) => ({ ...v, startDate: e.target.value }))} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" /></div>
          <div><label className="text-xs text-slate-500">End</label><input type="date" value={form.endDate} onChange={(e) => setForm((v) => ({ ...v, endDate: e.target.value }))} className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" /></div>
          ...
          <div className="md:col-span-5"><input value={form.reason} onChange={(e) => setForm((v) => ({ ...v, reason: e.target.value }))} placeholder="Reason for leave request" className="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" /></div>
        </div>
      </div>
      <div className="rounded-3xl border border-slate-200 bg-white p-4 space-y-3">
          ...
          return (
            <div key={r.id} className="rounded-2xl border border-slate-200 p-3 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
              <div className="text-sm">
                <div className="font-semibold text-slate-900">{staffName} • {r.type}</div>
                <div className="text-slate-600">{fmt(r.startDate)} - {fmt(r.endDate)} ({r.days} day{Number(r.days) > 1 ? 's' : ''})</div>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
          {staff.map((s) => (
            <div key={s.id} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm flex items-center justify-between">
              <span>{s.name}</span>
